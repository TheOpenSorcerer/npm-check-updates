...
const { isString, last } = require('../utils')
...
libnpmconfig.read().forEach((value, key) => {
  ...
  if (isString(pacoteKey)) {
    npmConfig[pacoteKey] = normalizedValue
  }
  else if (pacoteKey && typeof pacoteKey === 'function') {
...
module.exports = {
  ...
  async latest(packageName, currentVersion, options = {}) {
    ...
    return last(validVersions.map(o => o.version))
  },
